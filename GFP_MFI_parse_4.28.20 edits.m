...
% directory, use '.'
directory = '/Users/Christina/Documents/Ph.D 2015-2019/Lab Data/Microscopy/2.1.19 PA14 PA01 WT ladS Y1 Y2 check TP #2/PA14 WT 12H/'; 
...
%Load new fluorescence directory or directories
directory = '/Users/Christina/Documents/Ph.D 2015-2019/Lab Data/Microscopy/2.1.19 PA14 PA01 WT ladS Y1 Y2 check TP #2/PA14 WT 12H/GFP/';
datobj=dir([directory '*.dat']);
numfiles=length(datobj);

    daynum=1; %number of experimental days
...
for daynum=1 
   ...
   xaxis=linspace(min(gfpmfi),max(gfpmfi),binnum);
    ...
    figure(1)
    xlabel('Mean Fluorescence Intensity (AU)') %minus background
    ...
    hold on;
    bar(c,n, 'g') %if bars are desired
    %plot(c,n,'ok')%if data points are desired
    nfit=ecdfhist(cdfsum,x,binnum);
    plot(c,nfit,'-g')
    xlabel('Mean Fluorescence Intensity (AU)')
    ...
    legstr0='EM Fit';
    legstr1=['Est. %T3SS-OFF=' num2str(mdl.ComponentProportion(smallcomp)) ' \mu_{1}=' num2str(mdl.mu(smallcomp)) ' \sigma_{1}=' num2str(sqrt(mdl.Sigma(smallcomp))) ];
    legstr2=['Est. %T3SS-ON=' num2str(mdl.ComponentProportion(largecomp)) ' \mu_{2}=' num2str(mdl.mu(largecomp)) ' \sigma_{2}=' num2str(sqrt(mdl.Sigma(largecomp))) ];
    legend({'Histogram of GFP MFI',legstr1,legstr2})
    %legend({'combined data',legstr0,legstr1,legstr2})
    %title(['PDF evaluated with N=' num2str(binnum) 'bins']) %Title Optional
end